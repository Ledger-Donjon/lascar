...
container = BasicAesSimulationContainer(200, noise=2) # We use the BasicAesSimulationContainer with 2000 traces


def selection_function(value, guess):  # selection_with_guess function must take 2 arguments: value and guess
    """
...
session = Session(container, engine=cpa_engine, output_method=MatPlotLibOutputMethod(cpa_engine))
...
cpa_engines = [CpaEngine("cpa_%02d" % i, generate_selection_function(i), guess_range, solution=container.key[i]) for i in range(16)]

session = Session(container, engines=cpa_engines, name="cpa on 16 bytes", output_method= ScoreProgressionOutputMethod(*cpa_engines), output_steps=10).run(batch_size=50)
