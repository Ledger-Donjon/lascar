...
container = BasicAesSimulationContainer(10000,1) # We use the BasicAesSimulationContainer with 10000 traces
...
ttest_engines = [ TTestEngine("ttest_plaintext_%d"%i, get_partition_function(i)) for i in range(16)]

session = Session(container, engines=ttest_engines, output_method=MatPlotLibOutputMethod(*ttest_engines, single_plot=True, legend=True))
