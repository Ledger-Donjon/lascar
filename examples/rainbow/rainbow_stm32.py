...
from lascar import AbstractContainer, Trace, CpaEngine, Session, MatPlotLibOutputMethod, ScoreProgressionOutputMethod
...
class RainbowSubBytesContainer(AbstractContainer):
    ...
    def __init__(self, number_of_traces, key=[42]*16, noise=2, binary_file='code.elf', **kwargs):
        """
        ...
        value_dtype = np.dtype([('plaintext', np.uint8, (16,)),
                                ('key', np.uint8, (16,)),
                                ])
    ...
    def generate_trace(self,idx):
        ...
        address_for_input_buffer = 0xcafe00 # arbitrary value chosen for the input buffer address
        self.device[address_for_input_buffer] = bytes(self.value["plaintext"]) + bytes(self.value["key"]) # we build the input buffer for the subbytes function
        self.device['r0'] = address_for_input_buffer # first argument of the subbytes function
        self.device['r1'] = 16 # second argument of the subbytes function
        
        self.device.start(self.device.functions['subbytes'], 0) # rainbow launch the "subbytes" function

        # we prepare the leakage for this trace:
        leakage = np.fromiter(map(hamming_weight, self.device.sca_values_trace), dtype=np.uint8) # a hamming_weight model is applied to the rainbow-generated leakage
        leakage = np.random.normal(0,self.noise, len(leakage)) + leakage # artificial noise is added to the leakage
...
if __name__ == "__main__":
  ...
  engine = CpaEngine("cpa byte 5", lambda value,secret: sbox[ value["plaintext"][5] ^ secret], range(256), solution=42)
  session = Session(container,engine, output_method=ScoreProgressionOutputMethod(engine), output_steps=20).run()
