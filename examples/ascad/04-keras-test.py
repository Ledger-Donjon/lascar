...
if sys.argv[2] == "mlp":
    model_name = ASCAD_DIR + "/ASCAD_data/ASCAD_trained_models/mlp_best_ascad_desync0_node200_layernb6_epochs200_classes256_batchsize100.h5"
elif sys.argv[2] == "cnn":
    model_name = ASCAD_DIR + "/ASCAD_data/ASCAD_trained_models/cnn_best_ascad_desync0_epochs75_classes256_batchsize200.h5"
...
"""
attack_container = Hdf5Container(filename,"/Attack_traces/traces", "/Attack_traces/metadata", leakage_processing=ReshapeProcessing(nn.input_shape[1:]))

targeted_byte = 2
solution = attack_container[0].value['key'][targeted_byte] # The key byte we are looking for
...
nn_match_engine = MatchEngine("nn_match", nn, guess_function, guess_range, solution=solution)
...
for i in range(5):
    ...
    session = Session(container, name="run %d"%i, engine=nn_match_engine, output_method=RankProgressionOutputMethod(nn_match_engine, display=False), output_steps=range(0, 500, 10))
    session.run()

    ranks.append(session.output_method.get_scores_solution()['nn_match']) #ranks is filled with the progression of the rank of the solution for the engine 'nn_match'
