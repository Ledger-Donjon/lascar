...
Leakage Synchronisation is the name given to the post-processing method you apply on it to make all leakages "stackable"

In the following example we will design an AbstractContainer that will generate fake traces "desynchronized"
We will then design a Synchronisation method, which will consist in a leakage_processing applied to the AbstractContainer, so that it will deliver synchronized traces.
...
# We can use the plot_leakage() method to display the first traces, and note that the leakages are not stackable, as expected
...
The reason why we do that, is that our synchronisation process will attempt to overlay leakages based on a reference trace leakage.
...
but the soundness of the synchronisation is not the point here)
