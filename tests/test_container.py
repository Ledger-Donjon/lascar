...
leakage_processings = [lambda leakage: leakage,
...
class TestContainerLeakageProcessing:
    ...
    def test_leakage_processing_principal_component_analysis(self, container, number_of_components):
    ...
    def test_leakage_processing_centered_product(self, container, order):
        ...
        for i, combination in enumerate(leakage_processing.combinations):
            assert np.all(batch.leakages[:, i] == np.prod(leakages_centered[:, combination], axis=1))
