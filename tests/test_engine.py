...
hdf5_container = Hdf5Container.export(trace_batch_container, tempfile.mkdtemp()+"/tmp.h5")
multiple_container = MultipleContainer(TraceBatchContainer(leakages[:10], values[:10]),
...
class TestNonRegressionPartitionerEngine:
    @pytest.mark.parametrize('container,partition,partition_range',
    ...
    def test_snr_engine(self, container, partition, partition_range):
        ...
        means = np.zeros((len(partition_range),) + container_bis.leakages.shape[1:], dtype=np.double)
        vars = np.zeros((len(partition_range),) + container_bis.leakages.shape[1:], dtype=np.double)

        for i,val in enumerate(partition_range):
            idx = np.where(np.apply_along_axis(partition,1,container_bis.values) == val)[0]
    ...
    def test_nicv_engine(self, container, partition, partition_range):
        ...
        nicv_numpy = np.vstack(
            [container_bis.leakages[np.where(np.apply_along_axis(partition, 1, container_bis.values) == val)[0]].mean(0)
             for val in
             partition_range]).var(0) / container_bis.leakages.var(0)  # oui oui, promis

        assert np.all(np.isclose(nicv_numpy, engine.finalize()))


    @pytest.mark.parametrize('container,partition',
                             [(c, f[0]) for c in containers for f in functions_ttest])
    def test_ttest_engine(self, container, partition):
        ...
        container_bis = container[:]
        indexes = [np.where(np.apply_along_axis(partition,1,container_bis.values) == val)[0] for val in range(2)]
        ...
        ttest_numpy = (m0 - m1) / np.sqrt((v0 / len(indexes[0])) + (v1 / len(indexes[1])))
...
leakage_models = [
    HammingPrecomputedModel()
] + [BitLeakageModel(i) for i in range(2)]
...
class TestNonRegressionCpa:
    ...
    @pytest.mark.parametrize('container,guess_function, guess_range, jitv',
                             [(c, f[0], f[1], j) for c in containers for f in guess_functions for j in [True, False]])
    def test_cpa_engine(self, container, guess_function, guess_range, jitv):
        ...
        for i, guess in enumerate(guess_range):
            ...
            cpa_np[i] = np.array(
                [np.corrcoef(model, container_bis.leakages[:, j])[0, 1] for j in range(cpa_np.shape[1])])

        assert np.all(np.isclose(engine.finalize(), cpa_np)), "cpa non_regression test not passed."


    @pytest.mark.parametrize('container, partition, partition_size, guess_function, guess_range, leakage_model',
                             [(c, p, range(256), f[0], f[1], l) for c in containers for p in functions for f in guess_functions_for_partition for l in leakage_models])
    def test_cpa_partitioned_engine(self, container, partition, partition_size, guess_function, guess_range, leakage_model):
        ...
        for i, guess in enumerate(guess_range):
            model = np.array([leakage_model(guess_function(partition(d), guess)) for d in container_bis.values])
            cpa_np[i] = np.array(
                [np.corrcoef(model, container_bis.leakages[:, j])[0, 1] for j in range(cpa_np.shape[1])])
