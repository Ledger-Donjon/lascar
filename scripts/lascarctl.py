...
@click.option(
    ...
    default=10,
    type=int,
    help="indicate the number of traces you wish to acquire",
)
@click.option(
    "-v",
    "--value_getter",
...
@click.option(
    ...
    type=str,
    default=None,
    help="indicate the name for the output file (hdf5 for now).",
)
@click.option(
    "-b",
    "--batch_size",
...
@click.option(
    "-w", "--wait", default=0, type=float, help="delay in sec between trace requests"
)
@click.option(
    "-p",
    "--plot",
...
@click.option(
    ...
    nargs=2,
    default=None,
    help="indicate leakage_processing module_name and object_name",
)
@click.option(
    "-v",
    "--value_processing",
    nargs=2,
    default=None,
    help="indicate value_processing module_name and object_name",
)
@click.option(
    "-b",
    "--batch_size",
...
@click.option(
    ...
    default=0,
    type=int,
    help="indicate the number of traces you wish to process",
)
def processing(
    name_in,
    name_out,
    ...
    """
    leakage_processing = (
        import_object_from_module(*leakage_processing) if leakage_processing else None
    )
    value_processing = (
        import_object_from_module(*value_processing) if value_processing else None
    )
    ...
    if name_out:
        Hdf5Container.export(
            container, name_out, batch_size=10, name=name_out.split("/")[-1]
        )
    else:
...
@click.option(
    "-e", "--engines", default="engines", help="specify engines name within module"
)
@click.option(
    "-o",
    "--output_method",
    default="output_method",
    help="specify output_method name within module",
)
@click.option(
    "-b",
    "--batch_size",
...
@click.option(
    ...
    default=0,
    type=int,
    help="indicate the number of traces you wish to use ",
)
def run(name_in, module, engines, output_method, batch_size, number_of_traces):
    ...
    session = Session(container, engines=engines, output_method=output_method).run(
        batch_size
    )
...
@click.option(
    "-p", "--plot", is_flag=True, help="plot the computed ttests with matplotlib"
)
...
@click.option(
    ...
    default="leakages",
    type=str,
    help="Indicate the name of the leakage dataset",
)
@click.option(
    "-v",
    "--values_dataset",
    default="values",
    type=str,
    help="Indicate the name of the values dataset",
)
